...
function getStudentAverage(students){
        const reducir = students.map(estudiante =>{
            const resultado = estudiante.grades.reduce((calificaciones, valor) => calificaciones+valor,0);
            const obj = {
                name: estudiante.name,
                average: (resultado/estudiante.grades.length).toFixed(2)*1
            };
            return obj;
        });
        const promGeneral = reducir.reduce((acc,prom)=> acc+prom.average,0);
        let prom =(promGeneral/reducir.length).toFixed(2)*1
