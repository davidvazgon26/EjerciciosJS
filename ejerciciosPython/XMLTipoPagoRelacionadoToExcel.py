...
def extraer_informacion_xml(archivo):
    try:
        ...
        if tipo_comprobante == "P":
            ...
            # Obtener la información de los documentos relacionados
            documentos_relacionados = []
            for docto_relacionado in root.findall(".//{http://www.sat.gob.mx/Pagos20}DoctoRelacionado"):
                id_documento = docto_relacionado.attrib.get("IdDocumento")
                serie = docto_relacionado.attrib.get("Serie")
                folio = docto_relacionado.attrib.get("Folio")
                documentos_relacionados.append(f"{id_documento} | {serie} | {folio}")
            ...
            return {
                ...
                "UUID": uuid,
                "DocumentosRelacionados": ", ".join(documentos_relacionados)
            }
...
encabezados = ["Tipo de Comprobante", "Nombre Receptor", "RFC Receptor", "Monto Total Pagos", "Total Retenciones IVA", "Total Traslados Base IVA16", "Total Traslados Impuesto IVA16", "Fecha Pago", "Forma de Pago P", "Moneda P", "Monto", "Tipo Cambio P", "Fecha Timbrado", "No Certificado SAT", "UUID", "Documentos Relacionados"]
